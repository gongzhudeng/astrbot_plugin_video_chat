...
AstrBot 插件，让 AI 自动理解视频链接内容，支持抖音（视频/图文/动图）、B 站（含字幕提取）。
...
| 抖音 | 图文/动图帖 | `/share/note/`、`/share/slides/` 链接，多个素材按一个连续视频包抽帧理解 |
| B 站 | 视频 | BV号、av号、`b23.tv` 短链、完整链接，支持字幕提取 |

## 功能特性

- **无需命令**：基于 LLM Tool 模式运行，AI 自动判断何时解析视频，无需手动输入指令
- **抖音图文/动图支持**：自动识别图文和动图帖，多个素材共用同一个帧数预算，按一个连续内容包理解
...
```
抖音图文/动图帖（/share/note/、/share/slides/）
  → 原生 HTTP 提取素材列表
    → 下载素材 → 按 max_frames 共用一个总帧预算抽帧 → 发给视觉模型
